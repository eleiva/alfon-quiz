...
export async function GET() {
  try {
    const rows = (await sql`
      SELECT id, name, emoji, score, total, difficulty, pct,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, emoji, score, total, difficulty, pct } = body;
    ...
    const rows = (await sql`
      INSERT INTO leaderboard (name, emoji, score, total, difficulty, pct)
      VALUES (${safeName}, ${safeEmoji}, ${score}, ${total}, ${difficulty}, ${pct})
      RETURNING id, name, emoji, score, total, difficulty, pct,
                to_char(created_at AT TIME ZONE 'America/Argentina/Buenos_Aires', 'DD/MM') AS created_at
    `) as LeaderboardRow[];
